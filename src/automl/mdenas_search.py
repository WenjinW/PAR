...
from automl.par_basicmodel import BasicNetwork
from automl.darts_model import Network


class AutoSearch(object):
    # Implements a NAS methods MdeNAS
    def __init__(self, num_cells, num_class=10, input_size=None,
                init_channel=36, grad_clip=5, device='cuda: 0',
                exp_name=None, save_name='EXP', args=None):
        self.args = args
        ...
        self.lr = args.lr
        self.lr_a = args.lr_search
        self.weight_decay = args.lamb
    ...
    def search(self, t, train_data, batch_size, nepochs):
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, nepochs, eta_min=self.lr_min)
        ...
        for epoch in range(nepochs):
            ...
            # 2 train
            time1 = time.time()
            train_obj, train_acc, train_acc_5 = self.train(train_queue, selected_ops)
            time2 = time.time()
            valid_obj, valid_acc, valid_acc_5 = self.eval(valid_queue, selected_ops)
            time3 = time.time()
            ...
            for k in range(self.model.num_edges):
                ...
                for cell_type in ['normal', 'reduce']:
                    ...
                    self.model.p[cell_type][k] = F.softmax(self.model.p[cell_type][k], dim=0)
            time4 = time.time()
