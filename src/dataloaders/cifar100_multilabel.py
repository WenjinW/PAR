...
class MyCIFAR100(datasets.CIFAR10):
    ...
    def _load_meta(self) -> None:
        ...
        self.coarse_class_to_idx = {
            _coarse_class: i for i, _coarse_class in enumerate(self.coarse_classes)}
...
def get(path, seed=0, pc_valid=0.10, args=None):
    ...
    coarse_order = True if args.experiment == "cifar100_coarse" else False
    ...
    class_text_labels = train_dataset.classes

    print(class_text_labels)

    train_x, train_y = train_dataset.data, np.array(train_dataset.targets)
    val_x, val_y, train_x, train_y = _split_per_class(
        train_x, train_y, validation_split=pc_valid)
    ...
    if coarse_order:
        ...
        dataset_info_file_path = os.path.join(
            args.result_path, "dataset_info.txt")
        with open(dataset_info_file_path, 'w') as f:
            for line in dataset_info:
                f.write(line + '\n')
        args.logger.info("Write dataset information in {}".format(
    ...
    for i in range(num_tasks):
        train_x_i, train_y_i = _select(
            train_x, train_y, i*increments, (i+1)*increments)
        train_y_i -= i * increments
        val_x_i, val_y_i = _select(
            val_x, val_y, i*increments, (i+1)*increments)
        val_y_i -= i * increments
        test_x_i, test_y_i = _select(
            test_x, test_y, i*increments, (i+1)*increments)
