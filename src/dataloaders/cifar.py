...
def get(path, seed=0, pc_valid=0.10):
    ...
    if not os.path.isdir(data_path+'binary_cifar/'):
        os.makedirs(data_path+'binary_cifar')
        ...
        for t in data.keys():
            for s in ['train','test']:
                ...
                torch.save(data[t][s]['x'], os.path.join(os.path.expanduser(data_path+'binary_cifar'), 'data'+str(t)+s+'x.bin'))
                torch.save(data[t][s]['y'], os.path.join(os.path.expanduser(data_path+'binary_cifar'), 'data'+str(t)+s+'y.bin'))
    ...
    for i in range(10):
        ...
        for s in ['train', 'test']:
            data[i][s] = {'x':[],'y':[]}
            data[i][s]['x'] = torch.load(os.path.join(os.path.expanduser(data_path+'binary_cifar'),'data'+str(ids[i])+s+'x.bin'))
            data[i][s]['y'] = torch.load(os.path.join(os.path.expanduser(data_path+'binary_cifar'),'data'+str(ids[i])+s+'y.bin'))
