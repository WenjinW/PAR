...
class Appr(object):
    ...
    def train(self,t,xtrain,ytrain,xvalid,yvalid, device='cuda'):
        ...
        try:
            for e in range(self.nepochs):
                # Train
                clock0=time.time()
                self.train_epoch(t,xtrain,ytrain, device=device)
                clock1=time.time()
                train_loss,train_acc=self.eval(t,xtrain,ytrain, device=device)
                clock2=time.time()
                ...
                # Valid
                valid_loss,valid_acc=self.eval(t,xvalid,yvalid, device=device)
        ...
        # Activations mask
        task = torch.tensor(t, dtype=torch.long).to(device)
        # task=torch.autograd.Variable(torch.LongTensor([t]).cuda(),volatile=False)
        mask=self.model.mask(task, s=self.smax)
        for i in range(len(mask)):
            # mask[i]=torch.autograd.Variable(mask[i].data.clone(),requires_grad=False)
            mask[i] = torch.tensor(mask[i].data.clone(),requires_grad=False)
        if t == 0:
            self.mask_pre=mask
        else:
    ...
    def train_epoch(self,t,x,y,thres_cosh=50,thres_emb=6, device='cuda'):
        self.model.train()
