...
class Appr(object):
    ...
    def __init__(self,model,nepochs=100,sbatch=64,lr=0.05,lr_min=1e-4,lr_factor=3,lr_patience=5,clipgrad=1000,generations=20,args=None):
        ...
        self.P = 2  # 2 # from paper Secs 2.4 and 2.5, numbers of the individuals in each generation/paths to be trained
            ...
            for g in range(self.generations):
                ...
                for p in range(self.P):
                    ...
                    for e in range(self.nepochs):
                        # Train
                        clock0 = time.time()
                        self.train_epoch(t, xtrain, ytrain, Path[p], device=device)
                        clock1 = time.time()
                        train_loss, train_acc = self.eval(t, xtrain, ytrain, Path[p], device=device)
                        clock2 = time.time()
                        ...
                        # Valid
                        valid_loss,valid_acc = self.eval(t, xvalid, yvalid, Path[p], device=device)
                        # valid_loss,valid_acc=self.eval(t,xtrain,ytrain,Path[p], device=device)
                        ...
                        for j in range(self.L):
                            for k in range(self.N):
                                ...
                                if np.random.rand() < probability:
                                    # add int in [-2,2] to the path, this seems yet another hyperparam
                                    Path[p, j, k] = (Path[p, j, k]+np.random.randint(-2, 2)) % self.M
    ...
    def train_epoch(self, t, x, y, Path, device='cuda'):
        self.model.train()

        r = np.arange(x.size(0))
        np.random.shuffle(r)
        r = torch.LongTensor(r).to(device=device)
    ...
    def eval(self, t, x, y, Path=None, device='cuda'):
        total_loss = 0
        total_acc = 0
        total_num = 0
        self.model.eval()
