...
from automl.mdenas_search_parallel import AutoSearch
from models.TALL_model import Network
...
class Appr(object):
    ...
    def eval(self, t, test_loader, mode, device):
        ...
        with torch.no_grad():
            for x, y in test_loader:
                ...
                length = x.size()[0]
                if self.parallel:
                    outputs = self.parallel_model.forward(x)
                else:
    ...
    def learn_first(self, task_id, train_data, valid_data):
        ...
        # search a cell by mdenas and stack some cells to build a model, then train the model
        g = self.search_cell(train_data, valid_data,
        ...
        self.model = Network(self.input_size, self.task_class_num,
                             self.eval_layers, 36, g, self.device).to(self.device)

        if self.args.parallel:
            self.parallel_model = nn.DataParallel(self.model, device_ids=[0,1,2,3,4,5])
    ...
    def learn_new(self, task_id, train_data, valid_data):
        ...
        # 2 search task model
        # 2.1 freeze parameters of previous cells
        # utils.freeze_model(self.model)
        # self.model.modify_param(self.model.new_cells, True)
        # 2.2 select the task model
        self.search_model(task_id, train_data, valid_data,
                          self.o_batch, self.o_epochs)
        best_archi, genotype = self.model.select(task_id)
        self.archis.append(best_archi)
        self.cell_archis.append(genotype)

        # 3 train task model
        # 3.1 freeze parameters of previous cells
        # utils.freeze_model(self.model)
        # self.model.modify_param(self.model.model_to_train, True)
        # 3.2 train task model
    ...
    def search_model(self, task_id, train_data, valid_data, batch_size, epochs):
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            self.optimizer_o, epochs, eta_min=0.001)
        ...
        cell_e = [torch.full(pro.size(), 0, dtype=torch.long)
                  for pro in self.model.p_cell]
        cell_a = [torch.full(pro.size(), 0.0, dtype=torch.float)
                  for pro in self.model.p_cell]
        for cell_i_e in cell_e:
            cell_i_e[:-1] = self.o_size

        # 3.2 information of operations
        ops_e = {
        ...
        ops_a = {
            'normal': torch.full((self.model.num_edges, self.model.num_ops), 0.0, dtype=torch.float),
            'reduce': torch.full((self.model.num_edges, self.model.num_ops), 0.0, dtype=torch.float)
        }

        # 3 search the best model architecture
        for e in range(epochs):
            # 3.1 sample
            selected_cell = [torch.multinomial(
                pro, 1).item() for pro in self.model.p_cell]

            selected_ops = {
                'normal': torch.multinomial(self.model.p_ops['normal'], 1).view(-1),
                'reduce': torch.multinomial(self.model.p_ops['reduce'], 1).view(-1)
            }

            # 3.2 train
            time1 = time.time()
            train_loss, train_acc, train_acc_5 = self.search_epoch(
                task_id, train_loader, selected_cell, selected_ops, self.device)
            time2 = time.time()
            valid_loss, valid_acc, valid_acc_5 = self.search_eval(
                task_id, valid_loader, selected_cell, selected_ops, self.device)
            time3 = time.time()
            ...
            for i, idx in enumerate(selected_cell):
                cell_a[i][idx] = valid_acc
                if i > 0 and i < len(selected_cell) - 1:  # in the cell layers
                    # the new expansion cell is used
            ...
            if judge:
                for cell_type in ['normal', 'reduce']:
                    ...
                    for i, idx in enumerate(selected_ops[cell_type]):
                        ops_e[cell_type][i][idx] += 1
                        ops_a[cell_type][i][idx] = valid_acc

            # 3.4 update the probability
            for k in range(len(self.model.p_cell)):
                cell_de_k = torch.reshape(
                    cell_e[k], (1, -1)) - torch.reshape(cell_e[k], (-1, 1))
                cell_da_k = torch.reshape(
                    cell_a[k], (1, -1)) - torch.reshape(cell_a[k], (-1, 1))
            ...
            for k in range(self.model.num_edges):
                ops_de_k = {
                    'normal': torch.reshape(ops_e['normal'][k], (1, -1)) - torch.reshape(ops_e['normal'][k], (-1, 1)),
                    'reduce': torch.reshape(ops_e['reduce'][k], (1, -1)) - torch.reshape(ops_e['reduce'][k], (-1, 1))
                }
                ops_da_k = {
                    'normal': torch.reshape(ops_a['normal'][k], (1, -1)) - torch.reshape(ops_a['normal'][k], (-1, 1)),
                    'reduce': torch.reshape(ops_a['reduce'][k], (1, -1)) - torch.reshape(ops_a['reduce'][k], (-1, 1))
                }
                for cell_type in ['normal', 'reduce']:
                    # vector1 = torch.sum((dh_e_k[cell_type] < 0) * (dh_a_k[cell_type] > 0), dim=1)
                    vector1 = torch.sum(
                        (ops_de_k[cell_type] < 0) * (ops_da_k[cell_type] > 0), dim=0)
                    # vector2 = torch.sum((dh_e_k[cell_type] > 0) * (dh_a_k[cell_type] < 0), dim=1)
                    vector2 = torch.sum(
                        (ops_de_k[cell_type] > 0) * (ops_da_k[cell_type] < 0), dim=0)
                    self.model.p_ops[cell_type][k] += (
                        self.o_lr_a * (vector1-vector2).float())
                    self.model.p_ops[cell_type][k] = F.softmax(
                        self.model.p_ops[cell_type][k], dim=0)

            scheduler.step()

        return

    def search_epoch(self, t, train_loader, selected_cell, selected_ops, device='cuda'):
        self.model.train()
        total_loss = 0
        total_1_acc = 0
        total_5_acc = 0
        ...
        self.model.set_selected_model(selected_cell, selected_ops)
    ...
    def search_eval(self, t, test_loader, selected_cell, selected_ops, device):
        total_loss = 0
        total_1_acc = 0
        total_5_acc = 0
        total_num = 0
