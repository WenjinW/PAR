...
class Appr(object):
    ...
    def learn_first(self, task_id, train_data, valid_data):
        ...
        g = self.search_cell(train_data, valid_data, self.c_batch, self.c_epochs)
        self.cell_archis.append(g)
        # build the task model
        self.model = Network(self.input_size, self.task_class_num, self.eval_layers, 36, g, self.device).to(self.device)
        ...
        for e in range(epochs):
            ...
            for i, idx in enumerate(selected_cell):
                ...
                if i > 0 and i < len(selected_cell) - 1: # in the cell layers
                    if idx == self.model.length['cell'+str(i)]: # the new expansion cell is used
                        judge = True
                        cell_e[i][idx] += 1
                if i == 0:
            ...
            for k in range(self.model.num_edges):
                ...
                for cell_type in ['normal', 'reduce']:
                    ...
                    vector1 = torch.sum((ops_de_k[cell_type] < 0) * (ops_da_k[cell_type] > 0), dim=0)
                    # vector2 = torch.sum((dh_e_k[cell_type] > 0) * (dh_a_k[cell_type] < 0), dim=1)
                    vector2 = torch.sum((ops_de_k[cell_type] > 0) * (ops_da_k[cell_type] < 0), dim=0)
                    self.model.p_ops[cell_type][k] += (self.o_lr_a * (vector1-vector2).float())
                    self.model.p_ops[cell_type][k] = F.softmax(self.model.p_ops[cell_type][k], dim=0)
